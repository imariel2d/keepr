...
- **Partial (3):** change email (#27) — **backend built + verified end-to-end** (the mail-on flow +
  edge paths, against the Mailpit stack); the `/profile` change-email panel + `/confirm-email` page
  (Angular) and the mail-off live run remain ([feature-27-change-email.md](feature-27-change-email.md));
