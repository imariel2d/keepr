...
| 34 | Admin panel — account administration | ❌ | An admin view over **other** users: list accounts, disable/enable, reset a password, adjust quota, force sign-out, delete. The privileged counterpart to the self-service items above. **Hard prerequisite: a role/authorization model, which does not exist** — every account is an equal owner today (no `IsAdmin`/role on `User`, no `[Authorize(Roles=…)]`), so this needs a privilege flag plus a first-admin bootstrap (relates to Q-R3 in [registration-gate-design.md](registration-gate-design.md)). Force sign-out is nearly free given the `Sessions` table ([cookie-session-design.md](cookie-session-design.md) Q-C3); admin actions should be audited (#13/#22). This is the account-focused slice of the broader **#21** admin console |
